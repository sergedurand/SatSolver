...
public class SolverTwl_2 implements Solver {
	 ...
	 * With improved analysis of clauses
	 */
	@Override
		...
		while(it_cl.hasNext()) {
			...
			if(c.getLiterals().size()==2) {
				...
				while(it_lit.hasNext()) {
					int l3 = it_lit.next();
					if((l3 != l2) && (l3 != lit_id) && phi.getLiterals()[l3].getVal()==-1) {
						//found another watch
						it_cl.remove();
						phi.getWatched_literals()[l3].addClause(cl_id);
						c.getWatches()[l1_pos] = l3;
						found = true;
						break;
					}
				}
				if(found) {
					//we managed to find another watch and made the change
					continue; //move to next clause
				}else {
					if(phi.getLiterals()[l2].getVal() == -1) {// force to True and move on
						if(l2>= phi.getVariables().getSize()) {
							res.add(phi.getLiterals()[l2].getId()); 
